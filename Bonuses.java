...
public class Bonuses
{
	...
	{
		final int len_of_emp = points.length;
		boolean[] bonus_extra = new boolean[len_of_emp];
		int bonus_total = 0;
		int bonus_given = 0;
		int bonus_left = 0;
		double temp = 0;
		...
		for(int i=0;i<len_of_emp;i++)
			bonus_total+=points[i];


        for(int i=0;i<len_of_emp;i++)
        {
        	temp = (double)points[i]/bonus_total;
        	points[i] = (int)(temp*100);
        	bonus_given+= points[i];
        }

        for(int i=0;i<len_of_emp;i++)
			System.out.println(points[i]);
                    
        bonus_left = 100 - bonus_given;
        ...
        while(bonus_left>0)
        {
        	...
        	for(int i=0;i<len_of_emp;i++)
        	{
        		if(!bonus_extra[i] && points[i]>top_score)
        		{
        			top_score = points[i];
        			emp = i;
        		}
